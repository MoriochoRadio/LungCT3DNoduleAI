...
이 레포의 코드는 *완성된 결과물* 이 아니라 *학부 3학년 6명이 처음 의료 영상 AI 를 손으로 만지며 남긴 흔적* 그대로입니다. 일부러 다듬지 않은 것들:

### 1) `dhkstjd.pth` — 한글 IME 안 켠 메인 가중치
...
메인 결절 탐지 모델 가중치 파일명이 `dhkstjd.pth` 입니다. **"완성"** 을 입력하려다 한글 IME 를 안 켠 채로 영문 키를 눌러 생긴 흔적입니다. 정식 명명 (`model_final.pth`) 으로 바꾸지 않고 그대로 보존했습니다 — 학기 막바지에 *"이제 완성이다"* 하며 저장 버튼을 누른 학부생의 순간이 파일명에 박제되어 있습니다.

### 2) `tlqk` / `tlqkf.pth` — 두 번째·세 번째 IME 오타

```python
# notebooks/Untitled0.ipynb Cell 1
%cd /content/drive/MyDrive/tlqk
# ...
# Cell 3 L699
torch.save({...}, 'tlqkf.pth')
```

학습 노트북의 Colab 작업 폴더명이 `tlqk`, 체크포인트 저장 파일명이 `tlqkf.pth` 입니다. `dhkstjd` 에 이어 또 한 번 한글 IME 를 안 켠 흔적 (*"체크포인트/ckpt"* 입력 추정). 한 프로젝트 안에 같은 종류의 IME 오타가 세 번 등장하는 게 오히려 일관된 학부생 톤입니다.

### 3) `완성이다.ipynb` + `진짜완성/` — 학부생의 자기 격려

추론·평가·3D 시각화 노트북의 파일명이 [`notebooks/완성이다.ipynb`](notebooks/완성이다.ipynb) 입니다. 영어 정식명 (`inference.ipynb`) 으로 바꾸지 않고 한국어 원본명을 그대로 두었습니다. 흥미로운 건, 코드 백업 zip 안에는 `진짜완성/` 이라는 폴더가 또 있었다는 점입니다 — *"완성이다"* 만으로 부족했는지 *"진짜완성"* 으로 한 번 더 자기 격려를 한 흔적입니다. (백업 zip 은 중복이라 정리 과정에서 삭제했지만, 폴더명은 여기 박제해 둡니다.)

### 4) `test2.py` — 정식 명명 없는 진입점

Streamlit 진입점 파일명이 `app.py` 나 `streamlit_app.py` 가 아니라 `test2.py` 입니다. *"test1 다음의 test2"* 였을 이 파일이 그대로 최종 발표 시연에 쓰였습니다. 학부 데모는 진입점 이름을 정리할 여유까지는 없었다는 정직한 흔적입니다.

### 5) `부춸` 타이포 (test2.py L155)

```python
# src/test2.py L154-155
if approx_edges < min_edges:
    raise ValueError(f"❌ 메시 에지 수 부춸: {approx_edges}개")
```

*"부족"* 의 영구 타이포입니다. MedQueue 의 `Ture`/`Selectec`, SchoolbusRFID 의 `MFRC552`, ElderCaringApp 의 `roadcell` 에 이어 본인 포트폴리오 4 번째 영구 타이포로, 학습 흔적의 전통을 잇습니다.

### 6) 발표 슬라이드에 남은 to-do 메모

발표 PPT 안에 발표 준비 메모가 그대로 남아있습니다:
- **slide 39**: *"ppt 작년 구조로 바꾸기 / 이전 프로젝트 - 개선 방향 쪽으로 빼자"* — 발표 시나리오 정리 메모
- **slide 43**: *"- 수정 ㅇㅇㅇ"* — 슬라이드 안 미완성 to-do 표시

발표 직전까지 슬라이드를 다듬던 흔적입니다. MedQueue 의 `////수정` 마커와 같은 톤으로, 정리되기 전의 작업 과정을 그대로 보존했습니다.
